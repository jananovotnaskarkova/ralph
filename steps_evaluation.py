...
backgroundWithSteps = False

path = argv[1]
#folders = os.listdir(path) 
folders = ["2019_10_31_23_52_20"]

# finds out a hallway size
def count_hallway_size(dataset):
	hallwayInstance = f"{path}\\{dataset}\\hallwayInstance"
	m = 0
...
def count_steps(dataset, stepsCountZeros):
	...
	for y in range (0, 1000): 
		stateDump = pd.read_csv(f"{path}\\{dataset}\\evaluation\\episodeId_{y}\\stateDump")
